        ...
        with col_sync2:
            if st.button("🔄 立即同步", key="sync_kdocs_btn", use_container_width=True):
                with st.spinner("正在从石墨文档同步数据..."):
                    import subprocess
                    shimo_auth = st.secrets.get("SHIMO_AUTH", os.getenv("SHIMO_AUTH", ""))
                    if not shimo_auth:
                        st.error("❌ 未配置 SHIMO_AUTH，请在 Streamlit Cloud 的 Secrets 中设置")
                    else:
                        script = os.path.join(os.path.dirname(__file__), "sync_from_shimo.py")
                        result = subprocess.run(
                            [sys.executable, script],
                            capture_output=True, text=True, timeout=120,
                            env={**os.environ, "SHIMO_AUTH": shimo_auth, "PLAYWRIGHT_BROWSERS_PATH": "/tmp/ms-playwright"},
                        )
